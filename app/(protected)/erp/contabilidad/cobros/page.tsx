...
export default async function CobrosPage() {
  ...
  return <main className={styles.page}><div className={styles.container}><header className={styles.header}><h1>Cobros y Cheques</h1><p>Control de cuotas de clientes, anticipos y cartera de cheques diferidos.</p></header><ContabilidadTabs />{error && <div className={styles.error} role="alert">No se pudieron cargar los cobros: {error.message}</div>}<section className={styles.card}><div className={styles.cardHeader}><h2>Registrar cobro</h2><p>Cargá cuotas de cliente o cheques diferidos.</p></div><CobrosForm obras={obras} /></section><section className={styles.card}><div className={styles.cardHeader}><h2>Cartera registrada</h2><p>Seguimiento de anticipos, cuotas y cheques pendientes.</p></div><CobrosTable rows={rows} obras={obras} /></section></div></main>;
}
