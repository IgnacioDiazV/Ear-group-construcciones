...
export function WhatsAppImportModal({ obras }: { obras: ObraOption[] }) {
  ...
  function handleTextChange(value: string) {
    setTexto(value);
    setItems(parsearMensajeWhatsApp(value));
  }
  ...
  return (
    <>
      ...
      {open && <div className={styles.modalBackdrop} onClick={() => !pending && setOpen(false)} role="presentation"><div aria-labelledby="whatsapp-title" aria-modal="true" className={styles.modal} onClick={(event) => event.stopPropagation()} role="dialog">
        <div className={styles.modalHeader}><div><p className={styles.eyebrow}>Carga rápida</p><h2 id="whatsapp-title">Cargar desde WhatsApp</h2></div><button aria-label="Cerrar" className={styles.closeButton} disabled={pending} onClick={() => setOpen(false)} type="button">×</button></div>
        ...
        {items.length > 0 && <div className={styles.importTable}><div className={styles.importHeader}><span>Cantidad</span><span>Herramienta</span></div>{items.map((item, index) => <div className={styles.importRow} key={`${index}-${item.nombre}`}><input aria-label={`Cantidad de ${item.nombre}`} min="1" onChange={(event) => updateItem(index, "cantidad", event.target.value)} type="number" value={item.cantidad} /><input aria-label={`Nombre de herramienta ${index + 1}`} onChange={(event) => updateItem(index, "nombre", event.target.value)} value={item.nombre} /></div>)}</div>}
        {message && <div className={styles.error} role="alert">{message}</div>}
        <div className={styles.modalActions}><button className={styles.secondaryButton} disabled={pending} onClick={() => setOpen(false)} type="button">Cancelar</button><button className={styles.primaryButton} disabled={pending || !obraId || items.length === 0} onClick={submit} type="button">{pending ? "Registrando..." : "Registrar envío"}</button></div>
      </div></div>}
    </>
  );
}
